...
func check_if_overlaps_beacon() -> void:
	var elems = map_data.query_overlaps(entity.get_position(), { "include": beacon_manager.get_all_beacons(), "overlap_range": config.beacon_overlap_range })
	for node in elems:
		interact_with_beacon(node)

func interact_with_beacon(_node:PlayerBeacon) -> void:
	...
	if config.beacon_move_on_interact:
		beacon_manager.reposition_beacon()
